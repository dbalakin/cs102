...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    for row in range(len(solution)):
        row_val = set(get_row(solution, (row, 0)))
        if row_val != set('123456789'):
            return False

    for col in range(len(solution)):
        col_val = set(get_col(solution, (0, col)))
        if col_val != set('123456789'):
            return False

    for row in (0, 3, 6):
        for col in (0, 3, 6):
            blk_val = set(get_block(solution, (row, col)))
            if blk_val != set('123456789'):
                return False
